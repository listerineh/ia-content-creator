...
export function IntentSelector({ selectedIntents, onSelectionChange }: IntentSelectorProps) {
  const toggleIntent = (intentId: string, comingSoon?: boolean) => {
    if (comingSoon) return; // No permitir seleccionar si está próximamente
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {VIDEO_INTENTS.map(intent => (
        <IntentCard
          ...
          onClick={() => toggleIntent(intent.id, intent.comingSoon)}
        />
      ))}
    </div>
  );
...
function IntentCard({ intent, isSelected, onClick }: IntentCardProps) {
  ...
  const isDisabled = intent.comingSoon;
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <p className={cn('font-medium', isDisabled ? 'text-zinc-500' : 'text-white')}>
          {intent.name}
        </p>
